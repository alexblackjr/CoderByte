...
function checkCashRegister(price, cash, cid) {
 var totalCid = 0,
      change = cash - price,
      valueCompare = [100.00, 20.00, 10.00, 5.00, 1.00, 0.25, 0.10, 0.05, 0.01],
      amountOfEach = {},
      howMany = 0,
      getBack = [],
      first = 0,
      reverseMoneyOrder = arguments[2].reverse(),
      exactDrawer,
      remainingBalance;  
  
  
  //Cut off extra decimals
  function twoDecimals (z) {
      var strTotal = Math.fround(z).toString();
      var findDecimal = strTotal.indexOf(".");
      var cutoffPoint = findDecimal + 3; //show 3 decimal places
      if( parseInt(strTotal.substr(cutoffPoint,1)) >= 5){ //if 3rd decimal place is 5 or greater...
        var num = parseInt( strTotal.substr(cutoffPoint - 1,1) );
        num += 1;
        var injectNum = num.toString();
        var lastDigitSeen = cutoffPoint - 1;
        var shortNum = strTotal.replace(strTotal[lastDigitSeen], injectNum);//replace the 2nd decimal place
         return shortNum.substr(0,cutoffPoint);
      }else{ //return number as is and remove all numbers after 2 decimal places
 ...
 for(var a = 0; a < arguments[2].length; a++){
  ...
  exactDrawer = twoDecimals(totalCid);
   ...
   for(var j = 0; j < reverseMoneyOrder.length; j++){
     change = twoDecimals(change);
      if( (exactDrawer - change) < 0 ){
       return "Insufficient Funds";
      }else if( (exactDrawer - change) > 0 ){
