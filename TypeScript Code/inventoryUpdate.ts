...
function updateInventory(arr1: any[], arr2: any[]): any[] {

    // All inventory must be accounted for or you're fired!
    var checkPoint: number = ( arr2.length - 1 );
    var tempBox:[number, string];
    ...
    for(var i = 0; i < arr2.length; i++){
        ...
        for(var j = 0; j < arr1.length; j++){
            if(arr2[i][1] === arr1[j][1]){
                var x: number = arr2[i].shift();
                var y: number = arr1[j].shift();
                var combineResults: number = x + y;
    ...
    arr1.sort(function(a:string, b:string):any{
        return a[1] > b[1];
    });
